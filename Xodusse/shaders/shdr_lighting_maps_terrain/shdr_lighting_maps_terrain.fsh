...
Render main(Fragment INPUT)
{
    ...
    float4 P1 = gm_BaseTextureObject.Sample(Prop,U1*(UV3.zw-UV3.xy)+UV3.xy);
    float4 P2 = gm_BaseTextureObject.Sample(Prop,U2*(UV3.zw-UV3.xy)+UV3.xy);
    float4 P3 = gm_BaseTextureObject.Sample(Prop,U3*(UV3.zw-UV3.xy)+UV3.xy);
    float4 P4 = gm_BaseTextureObject.Sample(Prop,U4*(UV3.zw-UV3.xy)+UV3.xy);
    ...
    float4 C1 = gm_BaseTextureObject.Sample(Colr,U1*(UV1.zw-UV1.xy)+UV1.xy);
    float4 C2 = gm_BaseTextureObject.Sample(Colr,U2*(UV1.zw-UV1.xy)+UV1.xy);
    float4 C3 = gm_BaseTextureObject.Sample(Colr,U3*(UV1.zw-UV1.xy)+UV1.xy);
    float4 C4 = gm_BaseTextureObject.Sample(Colr,U4*(UV1.zw-UV1.xy)+UV1.xy);
    ...
    float4 N1 = gm_BaseTextureObject.Sample(Norm,U1*(UV2.zw-UV2.xy)+UV2.xy);
    float4 N2 = gm_BaseTextureObject.Sample(Norm,U2*(UV2.zw-UV2.xy)+UV2.xy);
    float4 N3 = gm_BaseTextureObject.Sample(Norm,U3*(UV2.zw-UV2.xy)+UV2.xy);
    float4 N4 = gm_BaseTextureObject.Sample(Norm,U4*(UV2.zw-UV2.xy)+UV2.xy);
    
    float3 Normal = normalize((N1*B.x+N2*B.y+N3*B.z+N4*B.w).xyz*2.0-1.0);
    OUTPUT.Norm = float4(Normal*.5+.5,Color.a);
    
    OUTPUT.Prop =  float4((P1*B.x+P2*B.y+P3*B.z+P4*B.w).rgb,Color.a);
    return OUTPUT;
}
